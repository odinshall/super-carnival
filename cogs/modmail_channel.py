...
class ModMailEvents(commands.Cog):
    ...
    async def send_mail_mod(self, message, prefix, anon: bool = False, msg: str = None):
        ...
        if data[9]:
            if str(await self.bot.tools.get_member_id(self.bot, message.channel)) in data[9].split(","):
            	return await message.channel.send(
               	 embed=discord.Embed(
        ...
        member = message.guild.get_member(await self.bot.tools.get_member_id(self.bot, message.channel))
        if member is None:
            return await message.channel.send(
                embed=discord.Embed(
        ...
        try:
            embed = discord.Embed(
                ...
                description=message.content[len('=areply'):].strip()
                if message.content.startswith('=areply')
                else message.content,
                colour=self.bot.mod_colour,
                timestamp=datetime.datetime.utcnow(),
            )
            # renamed all "anonymous#0000" to staff#0000 and member#0000
            embed.set_author(
                name=f"{message.author.name}#{message.author.discriminator}" if anon is False else "staff#0000",
