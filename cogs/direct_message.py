...
class DirectMessageEvents(commands.Cog, name="Direct Message"):
    ...
    async def send_mail(self, message, guild, to_send):
        ...
        # don't remember why i separated the two things, they were together in an "and"
        # surely i had a good reason but deleted it later :eyes:
        if data[9]:
            if str(message.author.id) in data[9].split(","):
	            return await message.channel.send(
              	  embed=discord.Embed(
        ...
        channels = [
            ...
            if checks.is_modmail_channel2(self.bot, channel, message.author)
            ]
        channel = None
        new_ticket = False
        if len(channels) > 0:
        ...
        if not channel:
            ...
            try:
                ...
                # not sure why but for some fuckin reason having this line of code outside the "try" sometimes fucked with the ticket amount and made it out of sync with actual tickets
                # this should probably be edited in a way that grabs the ticket ID from the database instead from the right row directly
                # ticket IDs are super important because they actually allow for blacklisting and whitelisting anonymously
                # can't afford mistakes with this one
                self.bot.total_tickets += 1
                if name:
                    name = name + f"-{message.author.discriminator}" if anon is False else f"ticket-{self.bot.total_tickets}"
                else:
                    name = message.author.id if anon is False else f"ticket-{self.bot.total_tickets}"
                channel = await category.create_text_channel(
                    name, topic=f"Ticket {self.bot.total_tickets}" if anon is False else None
                )
                c = self.bot.conn.cursor()
                c.execute(
                    "INSERT INTO tickets (guild, room, user, status, is_anon) VALUES (?, ?, ?, ?, ?)",
                    (str(guild.id), str(channel.id), str(message.author.id), 1, anon),
                )
                new_ticket = True
                ...
                if log_channel:
                    try:
                        ...
                        embed.set_footer(
                            text=f"{message.author.name}#{message.author.discriminator} | {message.author.id}"if anon is False else "member#0000",
        ...
        try:
            c = self.bot.conn.cursor()
            anon = checks.is_ticket_anon(self.bot, channel)
            ...
            embed.set_author(
                name=f"{message.author.name}#{message.author.discriminator}" if anon is False else "member#0000",
                icon_url=message.author.avatar_url
                if anon is False
                else "https://cdn.discordapp.com/embed/avatars/0.png",
            )
    ...
    async def select_guild(self, message, prefix, msg=None):
        ...
        for guild in guilds:
            channels = [
                channel for channel in guild.text_channels
				# the function being called here was edited a bit to accept the member object instead of the member ID
				# don't ask why
    ...
    async def on_message(self, message):
        ...
        if guild and confirmation is False:
            ...
            embed = discord.Embed(
				...
				# initially, i wanted to have two different messages based on whether or not...
    			# the user's message will be sent anonymously. but that was MUCH more difficult than anticipated
                f"If you have an open anonymous ticket in **{guild.name}**, this message will be sent to it anonymously, even if the server has anonymous messaging turned off.",
                colour=self.bot.primary_colour,
            )
